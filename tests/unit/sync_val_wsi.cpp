...
TEST_F(NegativeSyncValWsi, PresentBlockedByTimelineWait) {
    ...
    const VkPipelineStageFlags2 stage_mask = VK_PIPELINE_STAGE_2_COPY_BIT;

    m_default_queue->Submit2(m_command_buffer, vkt::Wait(acquire_semaphore),
                             vkt::TimelineSignal(timeline_semaphore, 1, stage_mask));
    ...
    m_default_queue->Submit2(vkt::no_cmd, vkt::TimelineWait(timeline_semaphore, 2, stage_mask),
                             vkt::Signal(submit_ready_semaphore, stage_mask));
