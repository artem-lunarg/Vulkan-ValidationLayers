...
TEST_F(PositiveSyncVal, RenderPassLayoutTransitionAndSecondaryCmdBuffer) {
    ...
    // Replace implicit EXTERNAL<->subpass dependency (which uses uses dstStageMask = ALL_COMMANDS)
    // with more narrow dependency. It is enough to synchronize only with the fragment shader accesses.
    // The LoadOp is NONE, that's why layout transition synchronizes with subpass acccesses and not
