...
VertexInputAccesses CommandBufferContext::CollectVertexAccesses(uint32_t vertex_count, uint32_t first_vertex) const {
    VertexInputAccesses result;
    result.pipeline = cb_state_->GetLastBoundGraphics().pipeline_state;
    ...
    const auto& vertex_bindings = result.pipeline->IsDynamic(CB_DYNAMIC_STATE_VERTEX_INPUT_EXT)
                                      ? cb_state_->dynamic_state_value.vertex_bindings
                                      : result.pipeline->vertex_input_state->bindings;
    for (const auto& [_, binding_state] : vertex_bindings) {
        ...
        if (const auto* vertex_buffer = vvl::Find(binding_buffers, binding_desc.binding)) {
            ...
            if (!buffer) continue;  // also skips if using nullDescriptor
            const AccessRange range =
                MakeRangeForVertexData(vertex_buffer->BufferOffset(), first_vertex, vertex_count, binding_state);
            result.accesses.push_back({buffer.get(), range});
        }
    }
    return result;
}

VertexInputAccesses CommandBufferContext::CollectIndexAccesses(uint32_t index_count, uint32_t first_index) const {
    VertexInputAccesses result;
    result.pipeline = cb_state_->GetLastBoundGraphics().pipeline_state;
    ...
    // vertices (new syncval mode). Scanning index buffer for each draw call might be the simplest option to implement
...
bool CommandBufferContext::ValidateDrawVertex(uint32_t vertex_count, uint32_t first_vertex, const Location& loc) const {
    const auto vertex_accesses = CollectVertexAccesses(vertex_count, first_vertex);
...
void CommandBufferContext::RecordDrawVertex(uint32_t vertex_count, uint32_t first_vertex, ResourceUsageTag tag) {
    auto vertex_accesses = CollectVertexAccesses(vertex_count, first_vertex);
...
bool CommandBufferContext::ValidateDrawVertexIndex(uint32_t index_count, uint32_t first_index, const Location& loc) const {
    const auto index_accesses = CollectIndexAccesses(index_count, first_index);
...
void CommandBufferContext::RecordDrawVertexIndex(uint32_t index_count, uint32_t first_index, ResourceUsageTag tag) {
    auto index_accesses = CollectIndexAccesses(index_count, first_index);
