...
std::string ErrorMessages::BufferError(const HazardResult& hazard, const CommandBufferContext& cb_context, vvl::Func command,
    ...
    return BufferError(hazard, cb_context.GetSyncEnvironment(), command, resource_description, range, std::move(additional_info));
}
...
std::string ErrorMessages::AccelerationStructureError(const HazardResult& hazard, const SyncEnvironment& env,
                                                      const vvl::Func command, const std::string& resource_description,
                                                      const AccessRange range, VkAccelerationStructureKHR as,
                                                      const std::string& as_field_description) const {
    AdditionalMessageInfo additional_info;

    std::ostringstream ss;
    ss << "The buffer backs ";
    ss << as_field_description;
...
std::string ErrorMessages::RenderPassAttachmentError(const HazardResult& hazard, const CommandBufferContext& cb_context,
    ...
    return RenderPassAttachmentError(hazard, cb_context.GetSyncEnvironment(), command, resource_description);
}

std::string ErrorMessages::RenderPassAttachmentError(const HazardResult& hazard, const SyncEnvironment& env, vvl::Func command,
                                                     const std::string& resource_description) const {
    return Error(env, hazard, command, resource_description, "RenderPassAttachmentError");
}
...
std::string ErrorMessages::BeginRenderingError(const HazardResult& hazard, const CommandBufferContext& cb_context,
    ...
    return BeginRenderingError(hazard, cb_context.GetSyncEnvironment(), command, resource_description, load_op);
}

std::string ErrorMessages::BeginRenderingError(const HazardResult& hazard, const SyncEnvironment& env, vvl::Func command,
...
std::string ErrorMessages::EndRenderingResolveError(const HazardResult& hazard, const CommandBufferContext& cb_context,
    ...
    return EndRenderingResolveError(hazard, cb_context.GetSyncEnvironment(), command, resource_description, resolve_mode,
                                    resolve_write);
}

std::string ErrorMessages::EndRenderingResolveError(const HazardResult& hazard, const SyncEnvironment& env, vvl::Func command,
...
std::string ErrorMessages::EndRenderingStoreError(const HazardResult& hazard, const CommandBufferContext& cb_context,
    ...
    return EndRenderingStoreError(hazard, cb_context.GetSyncEnvironment(), command, resource_description, store_op);
}

std::string ErrorMessages::EndRenderingStoreError(const HazardResult& hazard, const SyncEnvironment& env, vvl::Func command,
...
std::string ErrorMessages::RenderPassLoadOpError(const HazardResult& hazard, const SyncEnvironment& env, vvl::Func command,
...
std::string ErrorMessages::RenderPassLoadOpVsLayoutTransitionError(const HazardResult& hazard, const SyncEnvironment& env,
                                                                   vvl::Func command, const std::string& resource_description,
                                                                   VkAttachmentLoadOp load_op, bool is_color) const {
    AdditionalMessageInfo additional_info;
    const char* load_op_str = string_VkAttachmentLoadOp(load_op);
...
std::string ErrorMessages::RenderPassResolveError(const HazardResult& hazard, const SyncEnvironment& env, vvl::Func command,
                                                  const std::string& resource_description) const {
    return Error(env, hazard, command, resource_description, "RenderPassResolveError");
}

std::string ErrorMessages::RenderPassStoreOpError(const HazardResult& hazard, const SyncEnvironment& env, vvl::Func command,
                                                  const std::string& resource_description, VkAttachmentStoreOp store_op) const {
    AdditionalMessageInfo additional_info;
    const char* store_op_str = string_VkAttachmentStoreOp(store_op);
    additional_info.properties.Add(kPropertyStoreOp, store_op_str);
...
std::string ErrorMessages::RenderPassLayoutTransitionError(const HazardResult& hazard, const SyncEnvironment& env,
                                                           vvl::Func command, const std::string& resource_description,
...
std::string ErrorMessages::RenderPassLayoutTransitionVsResolveError(const HazardResult& hazard, const SyncEnvironment& env,
                                                                    vvl::Func command, const std::string& resource_description,
                                                                    VkImageLayout old_layout, VkImageLayout new_layout,
                                                                    uint32_t resolve_subpass, VkRenderPass render_pass) const {
    const char* old_layout_str = string_VkImageLayout(old_layout);
    const char* new_layout_str = string_VkImageLayout(new_layout);
...
std::string ErrorMessages::RenderPassFinalLayoutTransitionError(const HazardResult& hazard, const SyncEnvironment& env,
                                                                vvl::Func command, const std::string& resource_description,
                                                                VkImageLayout old_layout, VkImageLayout new_layout,
...
std::string ErrorMessages::RenderPassFinalLayoutTransitionVsStoreOrResolveError(
    const HazardResult& hazard, const SyncEnvironment& env, vvl::Func command, const std::string& resource_description,
    VkImageLayout old_layout, VkImageLayout new_layout, uint32_t store_resolve_subpass, VkRenderPass render_pass) const {
    const char* old_layout_str = string_VkImageLayout(old_layout);
    const char* new_layout_str = string_VkImageLayout(new_layout);
