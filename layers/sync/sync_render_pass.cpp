...
// The render pass validates detect some hazards against accesses that only exist inside the
// validation (kInvalidTag): the layout transition writes recorded on a temporary context and
// the store/resolve accesses of the proxy contexts. Those accesses are command buffer local,
// so record time validation reported the same hazard whenever it ran (a non-empty suppression
// range); regular hazards are filtered by the range itself.
static bool ReportedAtRecordTime(const HazardResult& hazard, const ResourceUsageRange& record_time_validated_tags) {
    if (hazard.Tag() == kInvalidTag) {
        return record_time_validated_tags.non_empty();
    }
...
class ValidateResolveAction {
  public:
    ValidateResolveAction(VkRenderPass render_pass, uint32_t subpass, uint32_t view_mask, const AccessContext& access_context,
                          const SyncEnvironment& env, const ResourceUsageRange& record_time_validated_tags, vvl::Func command)
    ...
    void operator()(const char* aspect_name, const char* resolve_action_name, uint32_t src_at, uint32_t dst_at,
        ...
        if (hazard.IsHazard() && !ReportedAtRecordTime(hazard, record_time_validated_tags_)) {
            ...
            const auto error = validator.error_messages_.RenderPassResolveError(hazard, env_, command_, resource_description);
...
bool RenderPassAccessContext::ValidateLayoutTransitions(const SyncEnvironment& env,
    ...
    for (const auto& transition : transitions) {
        ...
        if (hazard.IsHazard() && !ReportedAtRecordTime(hazard, record_time_validated_tags)) {
            ...
            if (hazard.Tag() == kInvalidTag) {
                const auto error = sync_state.error_messages_.RenderPassLayoutTransitionVsResolveError(
                    hazard, cb_context.GetSyncEnvironment(), command, resource_description, transition.old_layout,
                    transition.new_layout, transition.src_subpass, rp_state.VkHandle());
...
bool RenderPassAccessContext::ValidateLoadOperation(const SyncEnvironment& env,
                                                    const ResourceUsageRange& record_time_validated_tags,
                                                    const CommandBufferContext& cb_context, const AccessContext& access_context,
                                                    const vvl::RenderPass& rp_state, uint32_t render_pass_instance_id,
                                                    uint32_t subpass, uint32_t view_mask,
            ...
            if (hazard.IsHazard() && !ReportedAtRecordTime(hazard, record_time_validated_tags)) {
                ...
                if (hazard.Tag() == kInvalidTag) {  // Hazard vs. ILT
                    const auto error = sync_state.error_messages_.RenderPassLoadOpVsLayoutTransitionError(
                        hazard, cb_context.GetSyncEnvironment(), command, resource_description, load_op, is_color);
                    skip |= sync_state.SyncError(hazard.Hazard(), rp_state.Handle(), loc, error);
                } else {
                    const std::string error = sync_state.error_messages_.RenderPassLoadOpError(
                        hazard, cb_context.GetSyncEnvironment(), command, resource_description, subpass, i, load_op, is_color);
                    skip |= sync_state.SyncError(hazard.Hazard(), rp_state.Handle(), loc, error);
                }
            }
        }
...
bool RenderPassAccessContext::ValidateStoreOperation(const SyncEnvironment& env,
                                                     const ResourceUsageRange& record_time_validated_tags,
        ...
        if (auto filtered_view_mask = FilterViewMask(view_mask, current_subpass_, rp_state_->attachment_last_subpass[i])) {
            ...
            if (hazard.IsHazard() && !ReportedAtRecordTime(hazard, record_time_validated_tags)) {
                ...
                const std::string error =
                    sync_state.error_messages_.RenderPassStoreOpError(hazard, env, command, resource_description, store_op);
...
bool RenderPassAccessContext::ValidateResolveOperations(const SyncEnvironment& env,
                                                        const ResourceUsageRange& record_time_validated_tags,
                                                        const CommandBufferContext& cb_context, vvl::Func command) const {
    const uint32_t view_mask = rp_state_->create_info.pSubpasses[current_subpass_].viewMask;
    ValidateResolveAction validate_action(rp_state_->VkHandle(), current_subpass_, view_mask, CurrentContext(), env,
                                          record_time_validated_tags, command);
...
void RenderPassAccessContext::CollectDrawAttachmentAccesses(const vvl::CommandBuffer& cmd_buffer,
                                                            std::vector<DrawAttachmentAccess>& accesses) const {
    ...
    auto add_access = [&](const AttachmentViewGen& view_gen, AttachmentViewGen::Gen gen_type, SyncAccessIndex access_index,
                          const AttachmentAccess& attachment_access, const std::string& description) {
        const vvl::ImageView* view_state = view_gen.GetViewState();
        auto view = std::static_pointer_cast<const vvl::ImageView>(view_state->shared_from_this());
        if (subpass.viewMask == 0) {
            DrawAttachmentAccess access;
            access.view = view;
            access.range_gen = view_gen.GetRangeGen(gen_type);
            access.access_index = access_index;
            access.attachment_access = attachment_access;
            access.description = description;
            accesses.emplace_back(std::move(access));
        } else {
            uint32_t view_mask = subpass.viewMask;
            uint32_t view_index = 0;
            while (view_mask) {
                if (view_mask & 1) {
                    DrawAttachmentAccess access;
                    access.view = view;
                    access.range_gen = view_gen.GetRangeGen(gen_type, view_index);
                    access.access_index = access_index;
                    access.attachment_access = attachment_access;
                    access.description = description;
                    accesses.emplace_back(std::move(access));
    ...
    // Subpass's inputAttachment accesses are part of the descriptor accesses
    if (subpass.pColorAttachments && subpass.colorAttachmentCount && !list.empty()) {
        for (const auto location : list) {
            ...
            add_access(attachment_views_[subpass.pColorAttachments[location].attachment], AttachmentViewGen::Gen::kRenderArea,
                       SYNC_COLOR_ATTACHMENT_OUTPUT_COLOR_ATTACHMENT_WRITE, GetAttachmentAccess(SyncOrdering::kColorAttachment),
                       ss.str());
        }
    }
    ...
    if (depth_stencil_attachment != VK_ATTACHMENT_UNUSED) {
        ...
        if (IsDepthAttachmentWriteable(last_bound_state, ds_format)) {
            std::ostringstream ss;
            ss << "depth aspect of depth-stencil attachment in subpass " << cmd_buffer.GetActiveSubpass();
            add_access(view_gen, AttachmentViewGen::Gen::kDepthOnlyRenderArea,
                       SYNC_LATE_FRAGMENT_TESTS_DEPTH_STENCIL_ATTACHMENT_WRITE, attachment_access, ss.str());
        }
        if (IsStencilAttachmentWriteable(last_bound_state, ds_format)) {
            std::ostringstream ss;
            ss << "stencil aspect of depth-stencil attachment in subpass " << cmd_buffer.GetActiveSubpass();
            add_access(view_gen, AttachmentViewGen::Gen::kStencilOnlyRenderArea,
                       SYNC_LATE_FRAGMENT_TESTS_DEPTH_STENCIL_ATTACHMENT_WRITE, attachment_access, ss.str());
        }
    }
}
...
bool RenderPassAccessContext::ValidateNextSubpass(const SyncEnvironment& env, const ResourceUsageRange& record_time_validated_tags,
                                                  const CommandBufferContext& cb_context, vvl::Func command) const {
    // PHASE1 TODO: Add Validate Preserve attachments
    bool skip = false;
    skip |= ValidateResolveOperations(env, record_time_validated_tags, cb_context, command);
    skip |= ValidateStoreOperation(env, record_time_validated_tags, cb_context, command);
    ...
    skip |= ValidateLayoutTransitions(env, record_time_validated_tags, cb_context, next_context, *rp_state_,
    ...
    if (!skip) {
        ...
        skip |= ValidateLoadOperation(env, record_time_validated_tags, cb_context, temp_context, *rp_state_,
                                      render_pass_instance_id_, next_subpass, next_subpass_view_mask, attachment_views_, command);
    }
    return skip;
}
bool RenderPassAccessContext::ValidateEndRenderPass(const SyncEnvironment& env,
                                                    const ResourceUsageRange& record_time_validated_tags,
                                                    const CommandBufferContext& cb_context, vvl::Func command) const {
    ...
    skip |= ValidateResolveOperations(env, record_time_validated_tags, cb_context, command);
    skip |= ValidateStoreOperation(env, record_time_validated_tags, cb_context, command);
    skip |= ValidateFinalSubpassLayoutTransitions(env, record_time_validated_tags, cb_context, command);
    return skip;
}
        ...
        if (hazard.IsHazard() && !ReportedAtRecordTime(hazard, record_time_validated_tags)) {
            ...
            if (hazard.Tag() == kInvalidTag) {  // Hazard vs. store/resolve
                const std::string error = sync_state.error_messages_.RenderPassFinalLayoutTransitionVsStoreOrResolveError(
                    hazard, cb_context.GetSyncEnvironment(), command, resource_description, transition.old_layout,
                    transition.new_layout, transition.src_subpass, rp_state_->VkHandle());
                skip |= sync_state.SyncError(hazard.Hazard(), rp_state_->Handle(), loc, error);
            } else {
                const std::string error = sync_state.error_messages_.RenderPassFinalLayoutTransitionError(
                    hazard, cb_context.GetSyncEnvironment(), command, resource_description, transition.old_layout,
                    transition.new_layout, rp_state_->VkHandle());
                skip |= sync_state.SyncError(hazard.Hazard(), rp_state_->Handle(), loc, error);
            }
        }
    }
